...
NZ_AreaUnits_1996 <- NZ_AreaUnits_1996 %>%
  select(year, CAUnum96, CAUname96,
...
NZ_AreaUnits_1991 <- NZ_AreaUnits_1991 %>%
  separate(col = `CAU AUScore AUScale CauPopn`,
...
NZ_AreaUnits_1991 <- NZ_AreaUnits_1991 %>%
  pivot_longer(cols = c(AUScore, AUScale),
               names_to = "Indicator",
               values_to = "Value")
...
NZ_AreaUnits <- NZ_AreaUnits %>%
  pivot_longer(cols = c(CAU_avg_NZD, CAU_avg_NZD_score),
               names_to = "Indicator",
               values_to = "Value")
